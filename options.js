...
function save_options() {
	...
	chrome.storage.sync.set({
	 ...
	 $("#status").html("Options saved.").fadeIn(300);
	 setTimeout(function() {refreshPage();}, 1000);
	});
}
//to know if the form content has changed
$(function(){
   $(":input").change(function(){hasChanged = true;});
});
...
document.addEventListener('DOMContentLoaded', function() {
   document.getElementById('save').addEventListener('click',function() {callSaveOption();});
   document.getElementById('close').addEventListener('click',function() {window.close();});
   addTimeToSelectBox();
   restore_options();
});

function callSaveOption(){
	if(hasChanged){
		save_options()
	}else{
		$("#status").html("Nothing to save.").fadeIn(300).delay(650).fadeOut(300);;
	}
}
